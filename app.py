...
with col_left:
    with st.container(border=True):
        ...
        st.markdown(
            "<div style='color:#FAFAFA; font-size:16px; font-weight:700; margin-bottom:5px; margin-top:-5px;'>"
            "라인별 알람 분포</div>",
            unsafe_allow_html=True
        )
...
with col_right:
    with st.container(border=True):
        ...
        st.markdown('<div class="dist-card-marker"></div>', unsafe_allow_html=True)
        st.markdown(
            "<div style='color:#FAFAFA; font-size:16px; font-weight:700; margin-bottom:-10px;'>"
            "라인별 비율 (알람 건수 기준)</div>",
            unsafe_allow_html=True
        )
